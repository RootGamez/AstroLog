...
async def create_astrolog_record(record_in: AstrologRecordCreate, db: Session = Depends(get_db)):
    try:
        db_record = await create_record(db, record_in)
        return db_record
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

@router.get("/", response_model=List[AstrologRecordResponse])
def list_astrolog_records(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return get_records(db, skip=skip, limit=limit)

@router.get("/{record_id}", response_model=AstrologRecordResponse)
def get_astrolog_record(record_id: int, db: Session = Depends(get_db)):
...
def update_astrolog_record(record_id: int, record_in: AstrologRecordUpdate, db: Session = Depends(get_db)):
    db_record = update_record(db, record_id, record_in)
    if not db_record:
        raise HTTPException(status_code=404, detail="Registro no encontrado")
    return db_record

@router.delete("/{record_id}", response_model=AstrologRecordResponse)
def delete_astrolog_record(record_id: int, db: Session = Depends(get_db)):
    db_record = delete_record(db, record_id)
    if not db_record:
        raise HTTPException(status_code=404, detail="Registro no encontrado")
    return db_record
