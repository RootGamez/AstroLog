...
def create_favorite(db: Session, fav_in: MarsFavoriteCreate):
...
def get_favorites(db: Session):
    return db.query(MarsExploration).filter(MarsExploration.is_favorite == True).order_by(MarsExploration.created_at.desc()).all()

def delete_favorite(db: Session, fav_id: int):
    db_fav = db.query(MarsExploration).filter(MarsExploration.id == fav_id).first()
    if not db_fav:
        return None
    db.delete(db_fav)
    db.commit()
